...
const typedServices = services as readonly HomeServiceItem[];
...
export default function HomeHero() {
  const orderedServices = useMemo<HomeServiceItem[]>(() => {
    const servicesBySlug = new Map<string, HomeServiceItem>(
      typedServices.map((service) => [service.slug, service])
    );
    ...
    return ordered.length > 0 ? ordered : [...typedServices];
  }, []);
